...
// --- START: UPDATED LIVE SCORE CACHING LOGIC ---
const fetchLiveScores = async () => {
  try {
    const response = await axios.get(
      "https://cricket.sportmonks.com/api/v2.0/livescores",
    ...
    // Process each fetched match
    newScores.forEach((match) => {
      const index = liveScoresCache.findIndex((m) => m.id === match.id);

      if (index > -1) {
        // If match already exists in the cache, just update its data
        liveScoresCache[index] = match;
      } else {
        // If it's a new match, we need to add it
        // Check if the cache is already full
        if (liveScoresCache.length >= MAX_LIVE_MATCHES) {
          // Remove the oldest match (the first item in the array) to make space
          liveScoresCache.shift();
        }
        // Add the new match to the end of the array
        liveScoresCache.push(match);
      }
    });
  } catch (err) {
...
// --- END: UPDATED LIVE SCORE CACHING LOGIC ---
